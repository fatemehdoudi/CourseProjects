...
class CSABlock(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        B, L, C = x.size() #batch_size , prompt(sequence) length , n_embed
        # Q, K, V for all heads
        ### YOUR CODE HERE ###
        k = self.key(x).view(B , self.n_head , L , C // self.n_head)
        q = self.query(x).view(B , self.n_head , L , C // self.n_head)
        v = self.value(x).view(B , self.n_head , L , C // self.n_head)
        ### YOUR CODE HERE ###
        # Causal self-attention
        # attention dropout
        ### YOUR CODE HERE ###
        attn_save = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(C))
        attn_save = attn_save.masked_fill(self.mask[:, :, :L, :L] == 0, float('-inf'))
        attn_save = F.softmax(attn_save, dim=-1)
        att = self.attn_dropout(attn_save)
        ### YOUR CODE HERE ###
        ...
        ### YOUR CODE HERE ###
        # Readout projection
        y = self.resid_drop(self.proj(y)) #W^(0)
