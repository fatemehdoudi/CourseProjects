...
def generate_sample(model, tokenizer, conditions, max_length):
    ...
    with torch.no_grad():
        for _ in range(max_length - len_conditions):
            ...
            ids = torch.multinomial(probabilities, num_samples=1)
            if ids.item() == 2:
                break
            input_ids = torch.cat((input_ids, ids), dim=1)
        if (input_ids[0][len_conditions:]).shape[0] == 0:
            ids = torch.tensor([[3]], device='cuda:0')
            input_ids = torch.cat((input_ids, ids), dim=1)
            ### YOUR CODE HERE ###
    #print('decode: ', input_ids[0][len_conditions:])
    # print('input_ids: ' , input_ids)
    # print('input_ids[0][len_conditions:] : ' , input_ids[0][len_conditions:])
